...
const designHtmlInvited = `<!DOCTYPE HTML PUBLIC "-//W3C//DTD XHTML 1.0 Transitional //EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
<head>
<!--[if gte mso 9]>
<xml>
...
<table style="font-family:'Cabin',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <div style="font-size: 14px; color: #e5eaf5; line-height: 140%; text-align: center; word-wrap: break-word;">
    <p style="font-size: 14px; line-height: 140%;"><strong>Hi! {{firstName}}</strong></p>
  </div>
...
<div>
<div>
<p style="line-height: 160%;">{{email}} has accepted your invite to join the organization.</p>
</div>
</div>
</div>
  </div>
...
export class OrganizationService {
  ...
  async getInviteInfo(token: string) {
    ...
    if (!invitedMember) throw new NotFoundException('No invite found');
  ...
  async acceptInviteMember(u: IJwtPayload, token: string) {
    ...
    const content = designHtmlInvited
      .replace('{{firstName}}', `${inviter.firstName} ${inviter.lastName}`)
      .replace('{{firstName}}', `${inviter.firstName} ${inviter.lastName}`)
    ...
    await this.sendInviterAcceptedEmail(inviter, member, content);

    return this.authService.generateUserToken(user);
  }

  async sendInviterAcceptedEmail(
    inviter: UserEntity,
    member: MemberEntity,
    emailContent: string,
  ) {
